...
root = tkt.Tk()                                     #가장 기본적인 윈도우 창 root 가 생성
root.title("계산기")                                 #윈도우 창의 제목을 설정
root.resizable(0, 0)                                # 윈도우 크기 고정한다.
root.wm_attributes("-topmost", 1)                   # 윈도우의 속성을 지정한다.
...
calculation.grid(row=2, columnspan=8)               #객체의 배치를 담당.
def btnPress(num):
     global equa
     equa = equa + str(num)
     equation.set(equa)
