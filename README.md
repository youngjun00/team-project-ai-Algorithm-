...
## 문제점
> 현재 sympy 모듈변수를 출력하는 방법을 해결하지 못했습니다
> 이를 해결하기위해 sympy 모듈변수를 text 형식으로 변환후 출력을 해보면 어떨까라는 생각을하였으나 text 형식으로 변환되지가 않아 실패한 방법이였습니다
> sympy 내부변수를 변환시킬수 있는 방법만 찾는다면 해결할수있는 문제입니다.(12/8)

## 최종결과
> GUI 파트와 Database파트 및 코딩파트는 모두 연동이 된상태이고 sympy모듈 변수를 출력하는 방법만 찾으면 프로젝트를 성공적으로 완료 할수있을 것 같습니다.(12/8)
